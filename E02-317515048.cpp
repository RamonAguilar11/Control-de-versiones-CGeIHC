...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Inicializar matriz de dimensión 4x4 que servirá como matriz de modelo para almacenar las transformaciones geométrica
		...
		//Ventana izquierda
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-0.2f, 0.0f, -3.0f)); //Traslacion de posicionamiento 
		...
		//Puerta
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, -0.535f, -3.0f)); //Traslacion de posicionamiento 
		model = glm::scale(model, glm::vec3(0.33f, 0.33f, 0.0f)); //Escalamiento de la figura
		//
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model)); //FALSE ES PARA QUE NO SEA TRANSPUESTA y se envían al shader como variables de tipo uniform
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		...
		//Generando los triangulos verde 0.5f
		//Arbol izquierdo
		model = glm::mat4(1.0);
