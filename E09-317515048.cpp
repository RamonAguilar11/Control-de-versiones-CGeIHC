...
//Cambio visible de numeros
float Temporizador = 0.0f;
//Cambio de texturas 1 y 2
float TempoText = 0.0f;
bool textNum = true;
...
int main()
{
	...
	Tiamat_M = Model();
	Tiamat_M.LoadModel("Models/Tiamet_base.obj");
	Ala_Der_M = Model();
	...
	movCoche = 0.0f;
	movOffset = 0.1f; //Velocidad a la que se mueve el coche
	rotllanta = 0.0f;
	rotllantaOffset = 10.0f;
	rotAla = 0.0f;
	rotAlaOffset = 1.0f;
	avanza = true; //Condicional para la animacion  del coche
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::translate(model, glm::vec3(0.0f-angulovaria*0.01, 5.0f + 2.0*sin(glm::radians(angulovaria)), 6.0));
		...
		//Se agregan los numeros del 2 al 4
		for (int i = 1; i<4; i++)
		{
			//números 2-4
			toffsetnumerou += 0.25;
		...
		//Dibuja los numeros en distinto orden
		for (int j = 1; j < 5; j++)
		{
			//números 5-8
			toffsetnumerou += 0.25;
		...
		//número cambiante 
		/*
		¿Cómo hacer para que sea a una velocidad visible? -> Se agrega un temporizador
		*/
		//If para que toffsetnumerocambiau no se desborde
		//No se sale del rango de 0 a 1 del texturizado
		Temporizador += deltaTime * 0.025f;
		if (Temporizador >= 0.5f) { // cambia cada 0.5 segundos
			Temporizador = 0.0f; // reinicia el tiempo

			toffsetnumerocambiau += 0.25f;
			if (toffsetnumerocambiau > 0.75f) { //Se pone a 0.75 para que no se repita el 1 en el ciclo
		...
		TempoText += deltaTime * 0.025f;
		if (TempoText >= 1.0f) { 
			TempoText = 0.0f; // reinicia el tiempo
			textNum = !textNum; //Alterna entre true y false
		}
		toffsetnumerou = 0.0;
		toffsetnumerov = 0.0;
		toffset = glm::vec2(toffsetnumerou, toffsetnumerov);
		...
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		if (textNum) { 
			Numero1Texture.UseTexture(); //textNum -> True
		}
		else {
			Numero2Texture.UseTexture(); //textNum -> False
		}
		//if
