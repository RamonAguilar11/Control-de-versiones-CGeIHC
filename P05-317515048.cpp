...
int main()
{
	...
	//Importando los modelos del coche
	Porsche_M = Model();
	...
	while (!mainWindow.getShouldClose())
	{
		...
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = translate(model, glm::vec3(0.0f, 2.4f, mainWindow.getarticulacion3())); //Mueve el coche: adelante -> K, atrás -> L 
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Porsche_M.RenderModel(); //Modelo base del porsche

		//Cofre
		model = modelaux; 
		color = glm::vec3(0.7f, 0.7f, 0.7f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		model = translate(model, glm::vec3(0.0f, 3.3f, -8.8f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion1()), glm::vec3(1.0f, 0.0f, 0.0f)) ; //Mueve el coche: abre -> F, cierra -> G
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		modelaux = model;
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion2()), glm::vec3(1.0f, 0.0f, 0.0f)); //Mueve las llantas: adelante -> H, atrás -> J
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
