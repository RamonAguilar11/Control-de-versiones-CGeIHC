...
//para declarar varias luces de tipo pointlight/spotlight
...
int main()
{
	...
	pointLights[0] = PointLight(1.0f, 1.0f, 1.0f, //Color blanco para la luz
		0.25f, 1.0f,			//Intensidad de la luz
		-5.0f, 5.0f, -4.0,		//Posición
		0.3f, 0.05f, 0.01f);	//Atenuación
	pointLightCount++;
	//Luz puntual para Esfera Aural
	pointLights[1] = PointLight(0.0f, 0.7f, 1.0f, //Color blanco para la luz
	...
	while (!mainWindow.getShouldClose())
	{
		...
		spotLights[0].SetPos(glm::vec3(0.0f + mainWindow.getmuevex(), 0.5f, 1.0f)); //Luz del cofre
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		//*************************** Instancia Lampara ***************************************//
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-5.0f, 2.5f, -4.0));
